create database bi_dashboard ON CLUSTER cluster_group01;
...
) ENGINE = ReplicatedMergeTree('/clickhouse/tables/cluster_group01/{layer}-{sgroup01}/bi_dashboard/st_game_operate_channel_statistic', '{replica}') PARTITION BY timeline
ORDER BY (agg_dim, date_time, user_type, user_environment, agent_id, channel_type, channel_id, cid_id, billing_type, position, game_category, game_category_name, game_code, game_name);


CREATE TABLE bi_dashboard.st_game_operate_pruduct_quality_statistic ON CLUSTER cluster_group01(
`agg_dim` String COMMENT '聚合粒度',
 `date_time` Date COMMENT '时间',
 ...
 `real_roi` Float32 COMMENT '实际ROI',
 `timeline` String COMMENT '分区字段'
) ENGINE = ReplicatedMergeTree('/clickhouse/tables/cluster_group01/{layer}-{sgroup01}/bi_dashboard/st_game_operate_pruduct_quality_statistic', '{replica}') PARTITION BY timeline
ORDER BY (agg_dim, date_time, customer_id, customer_name, cooperate_type, game_category, game_category_name, game_code, game_name, agent_id, channel_type, channel_id);
...
`date_time` Date COMMENT '时间',
 ...
 `estimate_roi` Float32 COMMENT '预估ROI',
 `timeline` String COMMENT '分区字段'
) ENGINE = ReplicatedMergeTree('/clickhouse/tables/cluster_group01/{layer}-{sgroup01}/bi_dashboard/st_game_operate_roi_statistic', '{replica}') PARTITION BY timeline
 ORDER BY (date_time, kpi_id, group_name, category1, category2, category3, kpi_target, agent_id, channel_id, game_code, game_name);
...
) ENGINE = ReplicatedMergeTree('/clickhouse/tables/cluster_group01/{layer}-{sgroup01}/bi_dashboard/st_game_operate_kpi_statistic', '{replica}') ORDER BY id;
