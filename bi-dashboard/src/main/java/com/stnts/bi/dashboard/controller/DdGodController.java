...
@RestController
@RequestMapping("god")
public class DdGodController {

    @Autowired
    private BaseService baseService;

    @CheckPerm(authCode = AuthCodeEnum.DASHBOARD_BIZ_DDPW_PRODUCT_VIEW)
    @PostMapping("chart")
    public ResultEntity<QueryChartResultVO> chart(@RequestParam("data") String data) {
        return baseService.getChart(data);
    }

    @CheckPerm(authCode = AuthCodeEnum.DASHBOARD_BIZ_DDPW_PRODUCT_EXPORT)
