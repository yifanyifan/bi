...
/** 创建游戏运营数据库 */
create database bi_gameop on cluster cluster_bidev;
...
/**
  设备新增
 */
create table bi_gameop.dwd_new_dev on cluster cluster_bidev
(
...
) ENGINE = ReplicatedReplacingMergeTree('/clickhouse/tables/{layer}-{sgroup01}/bi_gameop/dwd_new_dev',
                                        '{replica}', update_time)
 ...
 * 用户归因
 */
create table bi_gameop.dwd_new_user on cluster cluster_bidev
(
    `business`     String comment '业务归属 yilewan、yfy',
    `game_code`    String comment '游戏CODE',
    ...
    `receive_time` UInt64 comment '时间戳(秒)',
    `update_time`  UInt64 comment '归因更新时间'
) ENGINE = ReplicatedReplacingMergeTree('/clickhouse/tables/{layer}-{sgroup01}/bi_gameop/dwd_new_user',
                                        '{replica}', update_time)
    PARTITION BY (business)
    ORDER BY
 ...
 * 角色归因
 */
create table bi_gameop.dwd_new_role on cluster cluster_bidev
(
    `business`     String comment '业务归属 yilewan、yfy',
    `game_code`    String comment '游戏CODE',
    ...
    `receive_time` UInt64 comment '时间戳(秒)',
    `update_time`  UInt64 comment '归因更新时间'
) ENGINE = ReplicatedReplacingMergeTree('/clickhouse/tables/{layer}-{sgroup01}/bi_gameop/dwd_new_role',
                                        '{replica}', update_time)
    PARTITION BY (business)
    ORDER BY
...
/**
  包含 用户 和  设备
 */
create table bi_gameop.ods_uc_login_info on cluster cluster_bidev
(
-- >> 分区字段
...
-- >> 扩展字段
    ...
    ENGINE = ReplicatedMergeTree('/clickhouse/tables/{layer}-{sgroup01}/bi_gameop/ods_uc_login_info',
...
/**
  包含角色 和 用户
 */
create table bi_gameop.ods_yilewan_game_role on cluster cluster_bidev
(
-- >> 分区字段
    `partition_date`     Date comment '分区字段',
-- >> 源数据
...
) ENGINE = ReplicatedMergeTree('/clickhouse/tables/{layer}-{sgroup01}/bi_gameop/ods_yilewan_game_role',
                               '{replica}') PARTITION BY (partition_date)
    ORDER BY
...
/**
  包含 设备 和 用户
 */
create table bi_gameop.ods_sdk_app_003 on cluster cluster_bidev
(
-- >> 分区字段
...
) ENGINE = ReplicatedMergeTree('/clickhouse/tables/{layer}-{sgroup01}/bi_gameop/ods_sdk_app_003',
                               '{replica}') PARTITION BY (partition_date)
    ORDER BY
(game_code, dev_id) SETTINGS index_granularity = 8192;


drop table bi_gameop.ods_bc_payment_info on cluster cluster_bidev sync;

/**
* ODS
*/
create table bi_gameop.ods_bc_payment_info on cluster cluster_bidev
(
-- >> 分区字段
    `partition_date`   Date comment '分区字段',
-- >> 源数据
...
) ENGINE = ReplicatedMergeTree('/clickhouse/tables/{layer}-{sgroup01}/bi_gameop/ods_bc_payment_info',
                               '{replica}') PARTITION BY (partition_date)
    ORDER BY
(business, game_code, pid, uid) SETTINGS index_granularity = 8192;
...
/**
  易乐玩和发行的同表结构, 不同表
 */
create table bi_gameop.dwd_publish on cluster cluster_bidev
(
...
) ENGINE = ReplicatedMergeTree('/clickhouse/tables/{layer}-{sgroup01}/bi_gameop/dwd_publish',
                               '{replica}') PARTITION BY (dim_type, partition_date)
    ORDER BY
(origin, business, game_code, pid, game_server) SETTINGS index_granularity = 8192;


create table bi_gameop.dws_publish_global on cluster cluster_bidev
(
-- >> 分区字段
    `dim_type`            String comment '统计类型：user.用户；dev.设备；role.角色',
    `partition_date`      Date comment '分区字段',
...
-- >> 付费
    ...
    ) ENGINE = ReplicatedMergeTree('/clickhouse/tables/{layer}-{sgroup01}/bi_gameop/dws_publish_global',
                                   '{replica}') PARTITION BY (dim_type, partition_date)
    ORDER BY
...
drop table bi_gameop.dw_publish_retention on cluster cluster_bidev sync;
/**
 * 留存表
 */
create table bi_gameop.dw_publish_retention on cluster cluster_bidev
(
    ...
    ) ENGINE = ReplicatedMergeTree('/clickhouse/tables/{layer}-{sgroup01}/bi_gameop/dw_publish_retention',
                                   '{replica}') PARTITION BY (ret_date)
    ORDER BY
(dim_type, new_date, business, pid, game_code, game_server) SETTINGS index_granularity = 8192;


/**
* 游戏发行聚合表
*/
create table bi_gameop.dws_publish_global_agg on cluster cluster_bidev
(
-- >> 分区字段
    `dim_type`            String comment '统计类型：user.用户；dev.设备；role.角色',
    `partition_date`      Date comment '分区字段',
...
-- >> 付费
    ...
    ) ENGINE = ReplicatedAggregatingMergeTree('/clickhouse/tables/{layer}-{sgroup01}/bi_gameop/dws_publish_global_agg',
                                              '{replica}')
...
drop table bi_gameop.dw_publish_retention_agg on cluster cluster_bidev sync;
/**
   * 聚合引擎的留存表
   */
create table bi_gameop.dw_publish_retention_agg on cluster cluster_bidev
(
    `ret_date`    Date comment '留存日期',
    `new_date`    Date comment '新增日期',
    `date_gap`    UInt8 comment '间距',
    ...
    `pay_money_total`   SimpleAggregateFunction(sum, Float64) comment '付费累积金额'
    ) ENGINE = ReplicatedAggregatingMergeTree('/clickhouse/tables/{layer}-{sgroup01}/bi_gameop/dw_publish_retention_agg',
                                              '{replica}')
    PARTITION BY (ret_date)
    ORDER BY
(dim_type, date_gap, business, game_code, pid, game_server) SETTINGS index_granularity = 8192;

alter table bi_gameop.dw_publish_retention_agg on cluster cluster_bidev add COLUMN `pay_money_total` SimpleAggregateFunction(sum, Float64) comment '总付费金额';



drop table bi_gameop.dw_publish_retention_2agg_mv on cluster cluster_bidev sync;
/**
* 留存的聚合引擎
*/
...
select ret_date,
       ...
       sum(pay_money_total)               as pay_money_total
from bi_gameop.dw_publish_retention
group by ret_date, new_date, date_gap, dim_type, business, game_code, pid, game_server ;


drop table bi_gameop.dw_publish_retention_init_2agg_mv on cluster cluster_bidev sync;
/**
 * 将0初始值同步到日留存表
 */
...
to bi_gameop.dw_publish_retention_agg
as
select addDays(new_arr, `number`) as ret_date ,
       new_date ,
       `number` as date_gap ,
       dim_type ,
       business ,
...
from (
         ...
         group by ret_date, new_date, date_gap, dim_type, business, game_code, pid, game_server ) as t1, numbers(0,181) as t2;

drop table bi_gameop.dw_publish_retention_total_2agg_mv on cluster cluster_bidev sync;

drop table bi_gameop.dw_publish_retention_total_2agg_mv on cluster cluster_bidev sync;

CREATE MATERIALIZED VIEW bi_gameop.dw_publish_retention_total_2agg_mv
on cluster cluster_bidev
to bi_gameop.dw_publish_retention_agg
as
select addDays(new_date , `number`) as ret_date ,
       new_date ,
       `number` as date_gap ,
       dim_type ,
       business ,
...
from (
         select ret_date,
                ...
                pid,
                game_server,
                sum(pay_money) as pay_money_total
         from bi_gameop.dw_publish_retention where pay_money > 0
         group by ret_date, new_date, date_gap, dim_type, business, game_code, pid, game_server ) as t1, numbers(0,181) as t2 where t2.`number` >= t1.date_gap ;

drop table if exists bi_gameop.dw_publish_retention_agg_week on cluster cluster_bidev sync;
/**
 * 周留存
 */
create table bi_gameop.dw_publish_retention_agg_week on cluster cluster_bidev
(
    `ret_date`    Date comment '留存日期',
    `new_date`    Date comment '新增日期',
    `date_gap`    UInt8 comment '间距',
    ...
    `active_arr`  SimpleAggregateFunction(groupUniqArrayArray, Array (String)) comment '活跃',
    `pay_money`   SimpleAggregateFunction(sum, Float64) comment '付费金额',
    `pay_money_total`   SimpleAggregateFunction(sum, Float64) comment '总付费金额'
    ) ENGINE = ReplicatedAggregatingMergeTree('/clickhouse/tables/{layer}-{sgroup01}/bi_gameop/dw_publish_retention_agg_week',
                                              '{replica}')
    PARTITION BY (ret_date)
    ORDER BY
(dim_type, date_gap, business, game_code, pid, game_server) SETTINGS index_granularity = 8192;
...
drop table bi_gameop.dw_publish_retention_2agg_week_mv on cluster cluster_bidev sync;
/**
...
drop table bi_gameop.dw_publish_retention_agg_month on cluster cluster_bidev sync;
/**
 * 月留存
 */
create table bi_gameop.dw_publish_retention_agg_month on cluster cluster_bidev
(
    `ret_date`    Date comment '留存日期',
    `new_date`    Date comment '新增日期',
    `date_gap`    UInt8 comment '间距',
    ...
    `active_arr`  SimpleAggregateFunction(groupUniqArrayArray, Array (String)) comment '活跃',
    `pay_money`   SimpleAggregateFunction(sum, Float64) comment '付费金额'
    ) ENGINE = ReplicatedAggregatingMergeTree('/clickhouse/tables/{layer}-{sgroup01}/bi_gameop/dw_publish_retention_agg_month',
                                              '{replica}')
    PARTITION BY (ret_date)
    ORDER BY
(dim_type, date_gap, business, game_code, pid, game_server) SETTINGS index_granularity = 8192;
...
drop table bi_gameop.dw_publish_retention_2agg_month_mv on cluster cluster_bidev sync;
/**
...
drop table bi_gameop.dws_publish_pay_analy on cluster cluster_bidev sync;

/**
 * 付费分析
 */
create table bi_gameop.dws_publish_pay_analy on cluster cluster_bidev(
                                                                         `partition_date` Date,
                                                                         `business` String comment '业务',
                                                                         `pid` String comment 'PID',
                                                                         `game_code` String comment '游戏code',
...
--    `charge_num` SimpleAggregateFunction(sum, UInt64) comment '充值次数',
--    `charge_money` SimpleAggregateFunction(sum, Float64) comment '充值金额'
                                                                         `charge_money_arr` SimpleAggregateFunction(groupUniqArrayArray, Array(Tuple(String, Float64))) comment '充值金额'
    ) ENGINE = ReplicatedAggregatingMergeTree(
                                                 '/clickhouse/tables/{layer}-{sgroup01}/bi_gameop/dws_publish_pay_analy',
                                                 '{replica}'
...
drop table bi_gameop.dwd_publish_2pay_analy_mv on cluster cluster_bidev sync;
/**
 *
 */
...
select
    ...
    game_server,
    id as uid,
--    count(1) as charge_num,
--    sum(pay_fee) as charge_money
...
create table bi_gameop.dws_publish_pay_analy_new on cluster cluster_bidev(
                                                                             `partition_date` Date,
                                                                             `business` String comment '业务',
                                                                             `pid` String comment 'PID',
                                                                             `game_code` String comment '游戏code',
                                                 ...
                                                 '/clickhouse/tables/{layer}-{sgroup01}/bi_gameop/dws_publish_pay_analy_new',
                                                 '{replica}'
                                             ) PARTITION BY (partition_date)
    ORDER BY
(business, pid, game_code, game_server, uid) SETTINGS index_granularity = 8192;

drop table bi_gameop.dwd_publish_2pay_analy_new_mv on cluster cluster_bidev sync;

create materialized view bi_gameop.dwd_publish_2pay_analy_new_mv
on cluster cluster_bidev
to bi_gameop.dws_publish_pay_analy_new as
select
    partition_date,
    business,
    pid,
    ...
    game_server,
    id as uid,
    groupUniqArray(tuple(pay_id, pay_fee)) as charge_money_arr
from
    bi_gameop.dwd_publish dp
where
        dim_type = 'user'
...
--create view bi_gameop.dws_publish_pay_analy_view on cluster cluster_bidev as
    --select
--    partition_date,
--    business,
--    pid,
...
drop table bi_gameop.dws_publish_dev_dis on cluster cluster_bidev sync;

/**
 * 设备分析
 */
create table bi_gameop.dws_publish_dev_dis on cluster cluster_bidev(
                                                                       `partition_date` Date,
                                                                       `business` String comment '业务',
                                                                       `pid` String comment 'PID',
                                                                       `game_code` String comment '游戏code',
                                                 ...
                                                 '/clickhouse/tables/{layer}-{sgroup01}/bi_gameop/dws_publish_dev_dis',
                                                 '{replica}'
                                             ) PARTITION BY (partition_date)
    ORDER BY
(business, pid, game_code, game_server) SETTINGS index_granularity = 8192;

drop table bi_gameop.dwd_publish_2dev_dis_mv on cluster cluster_bidev sync;

create materialized view bi_gameop.dwd_publish_2dev_dis_mv on cluster cluster_bidev to bi_gameop.dws_publish_dev_dis as
select
    partition_date,
    business,
    pid,
...
drop table bi_gameop.dws_publish_user_login on cluster cluster_bidev sync;
/**
 * 登录分析
 */
create table bi_gameop.dws_publish_user_login on cluster cluster_bidev(
                                                                          `partition_date` Date,
                                                                          `business` String comment '业务',
                                                                          `pid` String comment 'PID',
                                                                          `game_code` String comment '游戏code',
                                                                          `game_server` String comment '游戏区服',
                                                                          `uid` String,
--    `login_num` SimpleAggregateFunction(sum, UInt64) comment '登录次数'
                                                                          `login_arr` SimpleAggregateFunction(groupUniqArrayArray, Array(UInt64)) comment '登录次数'
    ) ENGINE = ReplicatedAggregatingMergeTree(
                                                 '/clickhouse/tables/{layer}-{sgroup01}/bi_gameop/dws_publish_user_login',
                                                 '{replica}'
                                             ) PARTITION BY (partition_date)
    ORDER BY
(business, pid, game_code, game_server, uid) SETTINGS index_granularity = 8192;

drop table bi_gameop.dwd_publish_2user_login_mv on cluster cluster_bidev sync;
create materialized view bi_gameop.dwd_publish_2user_login_mv on cluster cluster_bidev to bi_gameop.dws_publish_user_login as
select
    partition_date,
    business,
    pid,
...
drop table bi_gameop.dws_publish_pay_analy_view on cluster cluster_bidev sync;
/**
 * 付费分析视图
 */
create view bi_gameop.dws_publish_pay_analy_view on cluster cluster_bidev as
select
    ...
    pid.pid_alias as pid_alias,
    if(pid.channel_name is null or pid.channel_name = '', '自然量', pid.channel_name) as channel_name,
    if(pid.sub_channel_name is null or pid.sub_channel_name = '', '自然量', pid.sub_channel_name) as sub_channel_name,
    pid.medium_name as medium,
    if(pid.charge_rule is null or pid.charge_rule = '', '自运营', pid.charge_rule) as charge_rule,
    game.game_category_name as game_category_name,
    pid.pid_alias as batch,
    pid.pid_alias as account
from
...
drop table bi_gameop.dws_publish_dev_dis_view on cluster cluster_bidev sync;
/**
 * 机型分析视图
 */
create view bi_gameop.dws_publish_dev_dis_view on cluster cluster_bidev as
select
    partition_date,
    m.business as business,
    m.pid as pid,
...
drop table bi_gameop.dws_publish_user_login_view on cluster cluster_bidev sync;
/**
 * 用户登录视图
 */
create view bi_gameop.dws_publish_user_login_view on cluster cluster_bidev as
select
    partition_date,
    m.business as business,
    m.pid as pid,
...
/**
   * ==============================================分析
   */
drop table bi_gameop.dim_pid on cluster cluster_bidev;
CREATE TABLE bi_gameop.dim_pid on cluster cluster_bidev
(
    `pid`                   String,
    `pid_alias`             String,
...
drop table bi_gameop.dim_pid_cost on cluster cluster_bidev sync;
/**
 * PID消耗视图
 */
create table bi_gameop.dim_pid_cost on cluster cluster_bidev
(
    `id`           UInt64,
    `cost_date`    Date,
    `pid`          String,
...
(
    ...
    `book_cost`    Float64,
    `real_cost`    Float64
) ENGINE = MySQL('db-master-bi.stnts.com:33066',
                 'bi_pubop',
                 'dim_cost',
                 'bi_pubop_rw',
                 'ebtddf2gSRv32KVG');

--db-master-bi.stnts.com


drop table bi_gameop.dws_publish_global_agg_view on cluster cluster_bidev sync;
/**
  * 发行主视图
  */
create view bi_gameop.dws_publish_global_agg_view on cluster cluster_bidev as
...
drop table bi_gameop.dw_publish_retention_agg_view on cluster cluster_bidev sync;
/**
* 发行留存日视图
*/
create view bi_gameop.dw_publish_retention_agg_view on cluster cluster_bidev as
select ret_date ,
       new_date ,
       date_gap ,
       dim_type ,
       ...
       case when date_gap = 0 then ret.new_arr else ret.active_arr end as active_arr ,
       ret.pay_money as pay_money ,
       ret.pay_money_total as pay_money_total ,
       pid.username as create_username,
       game.game_name as game_name,
       pid.department_code as department_code,
       pid.company_name as company_name,
...
drop table bi_gameop.dw_publish_retention_agg_week_view on cluster cluster_bidev sync;
/**
 * 留存周视图
 */
create view bi_gameop.dw_publish_retention_agg_week_view on cluster cluster_bidev as
select ret_date ,
       new_date ,
       date_gap ,
       dim_type ,
...
drop table bi_gameop.dw_publish_retention_agg_month_view on cluster cluster_bidev sync;
/**
 * 留存月视图
 */
create view bi_gameop.dw_publish_retention_agg_month_view on cluster cluster_bidev as
select ret_date ,
       new_date ,
       date_gap ,
       dim_type ,
       ...
       pid.pid_alias as batch,
       pid.pid_alias as account
from bi_gameop.dw_publish_retention_agg_month ret
         left join bi_gameop.dim_pid pid on UPPER(ret.pid) = UPPER(pid.pid)
         left join bi_gameop.yilewan_dim_game_info game on ret.game_code = game.game_code;


truncate table bi_gameop.dw_publish_retention_pay_his_agg on cluster cluster_bidev sync;
/**
 * 将付费用户迁移到此处  付费用户的历史表
 */
create table bi_gameop.dw_publish_retention_pay_his_agg on cluster cluster_bidev
(
    `ret_date`      Date comment '留存日期',
    `dim_type`      String comment '统计类型：user.用户；dev.设备；role.角色',
    `business`      String comment 'business',
    ...
    `pay_arr`       SimpleAggregateFunction(groupUniqArrayArray, Array (String)) comment '付费',
    `new_pay_arr`   SimpleAggregateFunction(groupUniqArrayArray, Array (String)) comment '新增付费'
    ) ENGINE = ReplicatedAggregatingMergeTree('/clickhouse/tables/{layer}-{sgroup01}/bi_gameop/dw_publish_retention_pay_his_agg',
                                              '{replica}')
    PARTITION BY (ret_date)
    ORDER BY
(dim_type, business, game_code, pid, game_server) SETTINGS index_granularity = 8192;

drop table bi_gameop.dw_publish_retention_pay_his_2agg on cluster cluster_bidev sync;
/**
 * 将留存中付费 提取到
 */
 ...
 * 付费留存
 */
create table bi_gameop.dw_publish_retention_pay_agg on cluster cluster_bidev
(
    `ret_date`    Date comment '留存日期',
    `new_date`    Date comment '新增日期',
    `date_gap`    UInt8 comment '间距',
    ...
    ) ENGINE = ReplicatedAggregatingMergeTree('/clickhouse/tables/{layer}-{sgroup01}/bi_gameop/dw_publish_retention_pay_agg',
                                              '{replica}')
    PARTITION BY (ret_date)
    ORDER BY
(dim_type, date_gap, business, game_code, pid, game_server) SETTINGS index_granularity = 8192;
 ...
 * 新增付费留存
 */
--create table bi_gameop.dw_publish_retention_pay_new_agg on cluster cluster_bidev
    --(
--    `ret_date`    Date comment '留存日期',
--    `new_date`    Date comment '新增日期',
--    `date_gap`    UInt8 comment '间距',
...
--          (dim_type, date_gap, business, game_code, pid, game_server) SETTINGS index_granularity = 8192;
                                                ...
                                                drop table bi_gameop.dw_publish_retention_pay_2agg on cluster cluster_bidev sync;
/**
 * 物化视图写入付费用户留存
 */
                                                CREATE MATERIALIZED VIEW bi_gameop.dw_publish_retention_pay_2agg_mv on cluster cluster_bidev to bi_gameop.dw_publish_retention_pay_agg as
                                                select
                                                t_active.ret_date ,
  ...
  (
      ...
      groupUniqArrayArray(active_arr) as active_arr
    from
    bi_gameop.dws_publish_global_agg ret
    group by
    partition_date,
    dim_type,
    business,
 ...
 * 周付费留存
 */
    create table bi_gameop.dw_publish_retention_pay_agg_week on cluster cluster_bidev
  (
      `ret_date`    Date comment '留存日期',
      `new_date`    Date comment '新增日期',
      `date_gap`    UInt8 comment '间距',
    ...
    `active_arr`  SimpleAggregateFunction(groupUniqArrayArray, Array (String)) comment '付费活跃用户',
    `new_active_arr`  SimpleAggregateFunction(groupUniqArrayArray, Array (String)) comment '新增付费活跃用户'
    ) ENGINE = ReplicatedAggregatingMergeTree('/clickhouse/tables/{layer}-{sgroup01}/bi_gameop/dw_publish_retention_pay_agg_week',
                                              '{replica}')
    PARTITION BY (ret_date)
    ORDER BY
  (dim_type, date_gap, business, game_code, pid, game_server) SETTINGS index_granularity = 8192;
 ...
 * 月付费留存
 */
    create table bi_gameop.dw_publish_retention_pay_agg_month on cluster cluster_bidev
  (
      `ret_date`    Date comment '留存日期',
      `new_date`    Date comment '新增日期',
      `date_gap`    UInt8 comment '间距',
    ...
    `active_arr`  SimpleAggregateFunction(groupUniqArrayArray, Array (String)) comment '付费活跃用户',
    `new_active_arr`  SimpleAggregateFunction(groupUniqArrayArray, Array (String)) comment '新增付费活跃用户'
    ) ENGINE = ReplicatedAggregatingMergeTree('/clickhouse/tables/{layer}-{sgroup01}/bi_gameop/dw_publish_retention_pay_agg_month',
                                              '{replica}')
    PARTITION BY (ret_date)
    ORDER BY
  (dim_type, date_gap, business, game_code, pid, game_server) SETTINGS index_granularity = 8192;
    ...
    drop table bi_gameop.dw_publish_retention_pay_agg_view on cluster cluster_bidev sync;

    create view bi_gameop.dw_publish_retention_pay_agg_view on cluster cluster_bidev as
    select ret_date ,
    new_date ,
    date_gap ,
    dim_type ,
    ...
    drop table bi_gameop.dw_publish_retention_pay_agg_week_view on cluster cluster_bidev sync;

    create view bi_gameop.dw_publish_retention_pay_agg_week_view on cluster cluster_bidev as
    select ret_date ,
    new_date ,
    date_gap ,
    dim_type ,
    ...
    drop table bi_gameop.dw_publish_retention_pay_agg_month_view on cluster cluster_bidev sync;

    create view bi_gameop.dw_publish_retention_pay_agg_month_view on cluster cluster_bidev as
    select ret_date ,
    new_date ,
    date_gap ,
    dim_type ,
 ...
 * 为了消耗数据
 */
    create table bi_gameop.dws_publish_global_game_agg on cluster cluster_bidev
  (
-- >> 分区字段
      `dim_type`            String comment '统计类型：user.用户；dev.设备；role.角色',
      `partition_date`      Date comment '分区字段',
...
-- >> 付费
    ...
    `pay_first_num`       SimpleAggregateFunction(sum, UInt64) comment '首次付费次数',
    `pay_first_money`     SimpleAggregateFunction(sum, Float64) comment '首次付费金额'
    ) ENGINE = ReplicatedAggregatingMergeTree('/clickhouse/tables/{layer}-{sgroup01}/bi_gameop/dws_publish_global_game_agg',
                                              '{replica}')
    PARTITION BY (dim_type, partition_date)
    ORDER BY
  (business, game_code, pid) SETTINGS index_granularity = 8192;

/**
* 创建视图视图 合并dws层数据
*/
    create materialized view bi_gameop.dws_publish_global_game_2agg_mv
                                                          on cluster cluster_bidev
    to bi_gameop.dws_publish_global_game_agg as
    select dim_type,
    partition_date,
    business,
    pid,
    ...
    sum(pay_first_money) as pay_first_money
    from bi_gameop.dws_publish_global
    group by dim_type, partition_date, business, pid, game_code;


    drop table bi_gameop.dws_publish_global_game_agg_view on cluster cluster_bidev sync;

    create view bi_gameop.dws_publish_global_game_agg_view on cluster cluster_bidev
    as
    select dws.dim_type as dim_type,
    partition_date,
    dws.business as business,
    ...
    cost.book_cost book_cost,
    cost.real_cost real_cost
    from bi_gameop.dws_publish_global_game_agg dws
    left join bi_gameop.dim_pid pid on UPPER(dws.pid) = UPPER(pid.pid)
    left join bi_gameop.yilewan_dim_game_info game on dws.game_code = game.game_code
    left join bi_gameop.dim_pid_cost cost on dws.pid = cost.pid and dws.partition_date = cost.cost_date;
...
-- 游戏类型相关的字典表
    drop dictionary bi_gameop.st_games_list_dict on cluster cluster_bidev sync;
    CREATE DICTIONARY bi_gameop.st_games_list_dict on cluster cluster_bidev
  (

      `ab` String,
      `name` String,
      `gameEndType` UInt8,
      `gameUpdateTime` UInt64
  )
    PRIMARY KEY `ab`
    SOURCE(MYSQL(PORT 3306 USER 'Dssp_Setin_conn' PASSWORD 'chartdata_@)!#' REPLICA (HOST '47.100.34.42' PRIORITY 1) DB 'yilewan' TABLE 'st_games_list' INVALIDATE_QUERY 'select max(gameUpdateTime) from yilewan.st_games_list'))
    LIFETIME(MIN 2 MAX 60)
    LAYOUT(COMPLEX_KEY_HASHED());

    drop dictionary bi_gameop.fs_game_channel_dict on cluster cluster_bidev sync;
    CREATE DICTIONARY bi_gameop.fs_game_channel_dict on cluster cluster_bidev
  (
    ...
    SOURCE(MYSQL(PORT 33061 USER 'Dssp_Setin_conn' PASSWORD 'chartdata_@)!#' REPLICA (HOST '58.216.9.62' PRIORITY 1) DB 'fuse_sdk' TABLE 'fs_game_channel' INVALIDATE_QUERY 'select max(updated_at) from fuse_sdk.fs_game_channel'))
    LIFETIME(MIN 2 MAX 60)
    LAYOUT(COMPLEX_KEY_HASHED());

    drop table bi_gameop.yilewan_dim_game_info on cluster cluster_bidev sync;

    CREATE VIEW bi_gameop.yilewan_dim_game_info on cluster cluster_bidev as
    SELECT
    if(a.game_code != '',a.game_code,b.game_code) AS game_code,
    if(a.game_name != '',a.game_name,b.game_name) AS game_name,
    if(a.game_category_name != '',a.game_category_name,b.game_category_name) AS game_category_name,
    multiIf((a.business != '') AND (b.business != ''),'yilewan,yfy',(a.business != '') AND (b.business = ''),'yilewan',(a.business = '') AND (b.business != ''),'yfy',NULL) AS business
    FROM
  (
      SELECT
      upper(ab) AS game_code,
    upper(name) AS game_name,
    caseWithExpression(gameEndType,1,'页游',2,'端游',3,'手游',4,'H5',5,'其他','未知') AS game_category_name,
    'yilewan' AS business
    FROM bi_gameop.st_games_list_dict
    ) AS a
    FULL OUTER JOIN
  (
      SELECT
      upper(game_code) AS game_code,
    upper(max(game_name)) AS game_name,
    multiIf(max(game_platform) = 3,'H5',max(game_platform) IN (4,5),'手游',NULL) AS game_category_name,
    'yfy' AS business
    FROM bi_gameop.fs_game_channel_dict
    GROUP BY game_code
    ) AS b ON a.game_code = b.game_code;


    create view bi_gameop.app_publish_game_view on
    cluster cluster_bidev as
    select
    u.partition_date as partition_date ,
    u.game_code as game_code,
    u.new_arr as user_new_arr,
    --新增帐号
    u.active_arr as user_active_arr,
    -- 活跃帐号
    u.create_role_new_arr as user_create_role_new_arr,
    --新增创角账号数
    u.pay_total_money as user_pay_total_money,
    u.pay_total_unit_arr as user_pay_total_unit_arr,
    d.valid_new_arr as dev_valid_new_arr,
    d.new_arr as dev_new_arr,
    d.active_arr as dev_active_arr,
    r.new_arr as role_new_arr,
    r.active_arr as role_active_arr
    from
  (
      select
      partition_date,
    ...
    sum(pay_total_money) as pay_total_money
    from
    bi_gameop.dws_publish_global_game_agg dpgga
    where
    dim_type = 'user'
    and business = 'YFY'
    group by
    ...
    from bi_gameop.dws_publish_global_game_agg dpgga
    where
    dim_type = 'dev'
    and business = 'YFY'
    group by
    partition_date ,
    game_code ) d on
    u.partition_date = d.partition_date
    and u.game_code = d.game_code
    left join (
                  select
                  partition_date,
                  game_code,
    ...
    partition_date ,
    game_code ) as r on
    u.partition_date = r.partition_date
    and u.game_code = r.game_code;

    select * from bi_gameop.app_publish_game_view where partition_date = '2021-09-30';



    drop table bi_gameop.app_publish_game_view on cluster cluster_bidev sync;

    create view bi_gameop.app_publish_game_view on cluster cluster_bidev as
    select
    'YFY' as business,
    u.partition_date as partition_date,
    u.game_code as game_code,
    ...
    u.pay_total_money as user_pay_total_money,
    u.pay_total_unit_arr as user_pay_total_unit_arr,
    u.pay_new_unit_arr as user_pay_new_unit_arr,
    u.pay_new_money as user_pay_new_money,
    d.valid_new_arr as dev_valid_new_arr,
    d.new_arr as dev_new_arr,
    d.active_arr as dev_active_arr,
    r.new_arr as role_new_arr,
    ...
    pid.pid_alias as pid_alias,
    if(
          pid.channel_name is null
          or pid.channel_name = '',
          '自然量',
          pid.channel_name
      ) as channel_name,
    if(
          pid.sub_channel_name is null
          or pid.sub_channel_name = '',
          '自然量',
          pid.sub_channel_name
      ) as sub_channel_name,
    pid.medium_name as medium,
    if(
          pid.charge_rule is null
          or pid.charge_rule = '',
          '自运营',
          pid.charge_rule
      ) as charge_rule,
  ...
  (
    ...
    -- 活跃指标
                  groupUniqArrayArray(new_arr) as new_arr,
    groupUniqArrayArray(active_arr) as active_arr,
    groupUniqArrayArray(valid_new_arr) as valid_new_arr -- 付费
    from
    bi_gameop.dws_publish_global_game_agg dpgga
    where
    dim_type = 'dev'
    ...
    game_code,
    pid
    ) d on u.partition_date = d.partition_date
    and u.game_code = d.game_code
    and u.pid = d.pid
    left join (
                  select
    ...
    -- 活跃指标
                  groupUniqArrayArray(new_arr) as new_arr,
    groupUniqArrayArray(active_arr) as active_arr -- 付费
    from
    bi_gameop.dws_publish_global_game_agg dpgga
    where
    dim_type = 'role'
    ...
    game_code,
    pid
    ) as r on u.partition_date = r.partition_date
    and u.game_code = r.game_code
    and u.pid = r.pid
    left join bi_gameop.dim_pid pid on UPPER(u.pid) = UPPER(pid.pid)
    left join bi_gameop.yilewan_dim_game_info game on u.game_code = game.game_code;
