...
@Service
@StntsScheduleAnnatation(name = "plugin_merge_curr", cron = "0 */3 * * * ?", description = "商业插件今日数据合并")
public class PluginCurrMergeTask extends PluginMergeTaskBase implements IScheduleTask {
	
	private final SqlUtil sqlUtil;

    public PluginCurrMergeTask(SqlUtil sqlUtil) {
...
//		log.info("==>商业插件[插件]当日数据合并任务开始，周期：{}", date);
//		for(int x = 0 ; x <= 1 ; x++) {
//			for(int y = 0 ; y <= 1; y++ ) {
//				for(int z = 0 ; z <= 1; z++) {
